...
class TSSB(object):
    ...
    def __init__(self, dp_alpha=1.0, dp_gamma=1.0, root_node=None, 
        data=None, min_depth=0, max_depth=15, alpha_decay=1.0):
...
# 第3の動作
# nodeパラメータをHMCとSlicesamplingの併用によりサンプリング
...
# 第4の動作
    ...
    def make_category(self, max_category1, max_category2, max_category3):
        # root = self.root
        ct_0 = 0
        while ct_0 < max_category1:
            self.root['sticks'] = vstack([ self.root['sticks'], boundbeta(1, self.dp_gamma)])
            self.root['children'].append({ 'node' : self.root['node'].spawn(),
                                      'main'     : boundbeta(1.0, (self.alpha_decay**(1))*self.dp_alpha) if self.min_depth <= 1 else 0.0,
                                      'sticks'   : empty((0, 1)),
                                      'children' : []})
            ct_0 += 1
        for child in self.root['children']:
            ct_1 = 0
            while ct_1 < max_category2:
                child['sticks'] = vstack([ child['sticks'], boundbeta(1, self.dp_gamma)])
                child['children'].append({ 'node' : child['node'].spawn(),
                                           'main' : boundbeta(1.0, (self.alpha_decay**(2))*self.dp_alpha) if self.min_depth <= 2 else 0.0,
                                           'sticks' : empty((0, 1)),
                                           'children' : []})
                ct_1 += 1
            for child2 in child['children']:
                ct_2 = 0
                while ct_2 < max_category3:
                    child2['sticks'] = vstack([ child2['sticks'], boundbeta(1, self.dp_gamma)])
                    child2['children'].append({ 'node' : child2['node'].spawn(),
                                                'main' : boundbeta(1.0, (self.alpha_decay**(3))*self.dp_alpha) if self.min_depth <= 3 else 0.0,
                                                'sticks' : empty((0, 1)),
                                                'children' : []})
                    ct_2 += 1
        # self.root = root
    ...
    def add_pre_data(self, target, category_index):
        for n in xrange(0, len(self.data)):
            ...
            for col in list(category_index.columns)[1:]:
                indecies.append(int(tmp_df[col]))
            assignments = self.assignments

            def descend(root, indecies):
                current = root
                for index in indecies:
                    current = current['children'][index]
                current['node'].add_datum(n)
                assignments.append(current['node'])
            descend(self.root, indecies)
